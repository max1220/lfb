...
framebuffer_t newfb(const char* fbdev) {
    int fd;
	struct fb_fix_screeninfo finfo;
	struct fb_var_screeninfo vinfo;
	uint8_t *data;
    ...
    data = mmap(0, (vinfo.yres_virtual * finfo.line_length), PROT_READ | PROT_WRITE, MAP_SHARED, fd, (off_t)0);
...
void setpixel(framebuffer_t *fb, int x, int y, uint8_t r, uint8_t g, uint8_t b) {
    long location = (x + fb->vinfo.xoffset) * (fb->vinfo.bits_per_pixel / 8) + (y + fb->vinfo.yoffset) * fb->finfo.line_length;
    *((uint32_t*)(fb->data + location)) = ((r<<fb->vinfo.red.offset) | (g<<fb->vinfo.green.offset) | (b<<fb->vinfo.blue.offset));
}
...
static int lfb_new(lua_State *L) {
    ...
    framebuffer_t fb = newfb(luaL_checkstring(L, 1));
    lfb->fd = fb.fd;
    lfb->finfo = fb.finfo;
    lfb->vinfo = fb.vinfo;
    lfb->data = fb.data;
...
static int lfb_clear(lua_State *L) {
    ...
    int x;
    int y;
	uint8_t r = lua_tonumber(L, 1);
	uint8_t g = lua_tonumber(L, 2);
	uint8_t b = lua_tonumber(L, 3);
	uint32_t color = ((r<<lfb->vinfo.red.offset) | (g<<lfb->vinfo.green.offset) | (b<<lfb->vinfo.blue.offset));
	long location;

    for (x=0; x<lfb->vinfo.xres; x=x+1) {
        for (y=0; y<lfb->vinfo.yres; y=y+1) {
			location = (x + lfb->vinfo.xoffset) * (lfb->vinfo.bits_per_pixel / 8) + (y + lfb->vinfo.yoffset) * lfb->finfo.line_length;
			*((uint32_t*)(lfb->data + location)) = color;
        }
    }
...
static int lfb_setpixel(lua_State *L) {
    ...
    // setpixel(lfb, luaL_checkint(L, 2), luaL_checkint(L, 3), luaL_checkint(L, 4), luaL_checkint(L, 5), luaL_checkint(L, 6));
    setpixel(lfb, lua_tonumber(L, 2), lua_tonumber(L, 3), lua_tonumber(L, 4), lua_tonumber(L, 5), lua_tonumber(L, 6));
